...
def setup():
    ...
    parser.add_argument('--emb_enc_convs', type=int, default=4)
    parser.add_argument('--emb_enc_kersize', type=int, default=5)
    parser.add_argument('--mod_enc_convs', type=int, default=2)
    parser.add_argument('--mod_enc_kersize', type=int, default=7)
...
class BatchGen:
    ...
    def __init__(self, data, batch_size, gpu, evaluation=False):
        ...
        for i in range(batch_size - len(data[-1])):
            data[-1].append(random.choice(data[-1]))
        self.data = data
...
def score(pred, truth):
    ...
    pred = pred[:len(truth)] # we pad last batch with random elements from it
    f1 = em = total = 0
